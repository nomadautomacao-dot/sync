...
export function SecaoNaoMigrada({ titulo, fase, resumo }: SecaoNaoMigradaProps) {
  return (
    <div className="mx-auto max-w-[560px] px-8 py-16">
      <p className="font-mono text-[10px] font-semibold uppercase tracking-[1.1px] text-[#9CA3AF]">
        Fase {fase} da migração
      </p>
      <h1 className="mt-3 text-[23px] font-bold tracking-[-0.7px] text-[#111827]">{titulo}</h1>
      <p className="mt-2 text-[15px] leading-relaxed text-[#4B5563]">{resumo}</p>

      <div className="mt-6 rounded-[14px] border border-[#E2E8F0] bg-white p-5">
      ...
      <Link
        href="/painel"
        className="mt-6 inline-flex h-11 items-center rounded-[10px] bg-[#049598] px-5 text-[15px] font-semibold text-white transition-colors hover:bg-[#036B69]"
      >
        Voltar ao painel
      </Link>
    </div>
