...
test("admin cria cidade no próprio grupo", async () => {
  await assertSucceeds(
    setDoc(doc(ctx("u", admin), "cities/c2"), {
      groupId: "g1",
      name: "Nova",
      deletedAt: null,
    }),
...
test("membro do grupo cria cidade na carteira compartilhada", async () => {
  await assertSucceeds(
    setDoc(doc(ctx("u", member), "cities/c3"), {
      groupId: "g1",
      name: "Nova",
      deletedAt: null,
    }),
...
test("não cria cidade em outro grupo (hijack)", async () => {
  await assertFails(
    setDoc(doc(ctx("u", admin), "cities/c4"), {
      groupId: "g2",
      name: "Nova",
      deletedAt: null,
    }),
...
test("admin pode excluir cidade de forma recuperável", async () => {
  await seedCity("c1", "g1");
  const { updateDoc, serverTimestamp } = await import("firebase/firestore");
  await assertSucceeds(
    updateDoc(doc(ctx("u", admin), "cities/c1"), {
      status: "excluido",
...
test("membro não pode marcar cidade como excluída", async () => {
  await seedCity("c1", "g1");
  const { updateDoc, serverTimestamp } = await import("firebase/firestore");
  await assertFails(
    updateDoc(doc(ctx("u", member), "cities/c1"), {
      status: "excluido",
      deletedAt: serverTimestamp(),
    }),
  );
...
test("membro do grupo edita cidade da carteira compartilhada", async () => {
  await seedCity("c1", "g1");
  const { updateDoc } = await import("firebase/firestore");
  await assertSucceeds(
    updateDoc(doc(ctx("u", member), "cities/c1"), {
...
test("admin de outro grupo NÃO edita cidade de g1 por doc-id (cross-group)", async () => {
  ...
  await assertFails(
    updateDoc(doc(ctx("u", other), "cities/c1"), {
      name: "Y",
      groupId: "g1",
    }),
  );
